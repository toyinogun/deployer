...
- Anything running inside the cluster reaches the platform on `DEPLOYER_INTERNAL_URL`, because a build pod resolves names on cluster DNS, which cannot see a public hostname. A public URL is only ever for text handed to a caller. There are two of them and **neither is configured directly**: spec 0022 removed `DEPLOYER_PUBLIC_URL` outright, so the deploy path's address is derived from `DEPLOYER_MCP_HOST` and the address a person clicks in a verification, reset or invite mail is derived from `DEPLOYER_CONSOLE_HOST`. Each host and its URL therefore cannot disagree, and a boot that still sets `DEPLOYER_PUBLIC_URL` fails naming it rather than ignoring it, because a stale ConfigMap that is merely ignored goes on looking correct.
- A route that changes cluster state is absent from the mux on the console hostname, rather than being refused by a check inside it (spec 0021). Registration is opt in: every public route is registered a second time under the console host pattern and one catch all answers 404 for the rest, so a new route nobody remembers to register twice is private. Spec 0022 gave the deploy path a third pattern of the same shape, `DEPLOYER_MCP_HOST`, carrying exactly `POST /v1/uploads` and `/mcp`: the single use fetch route stays off it deliberately, since only an in cluster build pod reads that one. `CF-Connecting-IP` is then read on those two public hostnames and nowhere else, and what makes that safe is not the header but the path: each public origin is the `deployer` Service directly rather than the shared ingress controller, so network policy can name the tunnel namespace as the only outside peer on 8080, and a second public name adds no peer. Moving either one behind `ingress-nginx` makes the header writable from most of the cluster, and nothing in the suite would notice. Spec 0024 then broke the console's own half of that description without weakening the mechanism: the console hostname now serves three machine endpoints, `POST /oauth/register`, `POST /oauth/token` and the authorization server metadata document, because the approval page needs the `__Host-` session cookie and can live nowhere else. Read "the console is browser only" as retired; what still holds is that registration is opt in and a route nobody registers twice is private.
- Every rule a sign in is refused by lives in `Service.Login` in `internal/identity`, never in a handler. Two surfaces call it, the browser forms in `internal/web` and the JSON routes in `internal/httpapi`, and a rule only one of them applies is not a rule (spec 0021, AC-5). The lockout was in the JSON handler alone until 2026-08-16: it checked `LockedOut` and called `Failed` itself, `Service.Login` touched the limiter nowhere, and the browser therefore counted no failures and honoured no penalty, while a comment there claimed it inherited both. Nothing caught it, because each surface's tests only ever asserted its own behaviour. Spec 0021 is what made it bite, since the JSON surface is 404 on the console hostname, so the surface with no lockout became the only one on the open internet. Two consequences to keep: a refusal added to a handler is a bug even when that handler is the only one you are looking at, and a rule moved into `Login` has to be **removed** from the handler rather than left in both, or one wrong password counts twice on that surface alone. The per address rate limit bucket is the deliberate exception, spent by the handler through `spend`, because it bounds the call rather than judging the credentials.
...
- The OAuth flow is the one written exception to the closed code sets, scoped to the three `/oauth/` routes and recorded in spec 0024 AC-24: they answer RFC 6749 error codes because a connector client parses that vocabulary and understands nothing else. It is not licence for a second code set anywhere else. Within it, every way an exchange can fail is the single `ErrGrantInvalid`, so a refusal never says which check refused it, and a replay's PKCE verifier alone decides whether the token it issued is revoked (AC-16b). That last one depends on the order of the checks in `identity.Service.Grant`: the consumed branch answers before the client id, redirect URI and resource are compared, and moving it restores a bug where an ordinary retry cost a connector its credential.
...
- Source reaches the platform two ways and there is only one path underneath. Spec 0026 gave `deploy_app` an inline `files` map, because a connector client with no shell can call every tool but cannot make the upload request the `upload_id` needed. `uploads.Pack` composes the archive and `uploads.Service.Accept` records it, so the inline call spends the same unclaimed slot, meets the same ceiling in the same `upload_too_large`, expires in the same window and is cleared by the same sweep. A bound added to the upload route rather than to the service is a bound the inline caller walks past.
- An MCP tool's description is part of the contract, not decoration. `deploy_app`'s carries both ways of giving the source and the rules an app must meet, so a change to either is a change to the description in the same commit. Since spec 0022 the two values most likely to drift are derived rather than written: the endpoint comes from `DEPLOYER_MCP_HOST` and the stated ceiling from the configured `MaxUploadBytes`, and `description_test.go` changes the configuration and asserts the text follows (AC-10). The prose around them is still untested.
...
- A test that pairs `httptest` with a client on the default transport is not isolated from the other tests in its package. `httptest.Server.Close` calls `CloseIdleConnections` on `http.DefaultTransport`, which every `&http.Client{}` shares, so one parallel test shutting its server down breaks a connection another test is reusing, and the failure lands in whichever test happened to be mid request. It reads as that test's own bug and reproduces on CI far more often than locally. `internal/tunnelwatch` lost a CI run to it on 2026-08-16, where a healthy endpoint read as an outage: give each test its own `&http.Transport{}` and close it on cleanup.
...
- [internal/domain/AGENTS.md](internal/domain/AGENTS.md): the pure rules layer, the three closed code sets and why they stay separate, and the hand written list in the reason tests that has to be updated with the codes.
- [internal/uploads/AGENTS.md](internal/uploads/AGENTS.md): the source tarball's whole life, the two size gates, the single use fetch token, and the sweep's deliberate file before row order.
- [internal/mcp/AGENTS.md](internal/mcp/AGENTS.md): the agent facing tools, why the handler observes and never acts, where a refusal is allowed to live, and why a handler test alone does not cover a reason code.
