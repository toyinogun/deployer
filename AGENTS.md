...
- Anything running inside the cluster reaches the platform on `DEPLOYER_INTERNAL_URL`. `DEPLOYER_PUBLIC_URL` is only for text handed to a caller, because a build pod resolves names on cluster DNS, which cannot see the public hostname. Since spec 0021 it is narrower still: it is the tailnet address an agent is told to upload to, and the address a person clicks in a verification, reset or invite mail is derived from `DEPLOYER_CONSOLE_HOST` instead. The console's own base URL is derived, never configured, so the two cannot disagree.
- A route that changes cluster state is absent from the mux on the console hostname, rather than being refused by a check inside it (spec 0021). Registration is opt in: every public route is registered a second time under the console host pattern and one catch all answers 404 for the rest, so a new route nobody remembers to register twice is private. `CF-Connecting-IP` is then read on that one hostname and nowhere else, and what makes that safe is not the header but the path: the console origin is the `deployer` Service directly rather than the shared ingress controller, so network policy can name the tunnel namespace as the only outside peer on 8080. Moving the console behind `ingress-nginx` makes the header writable from most of the cluster, and nothing in the suite would notice.
