...
- [ ] Build it: `/develop connecting a client that will not hold a token`
  - [ ] The schema and the grant path: migration `00007` with both tables, the nullable column and the two indexes that carry rules, plus the store transaction and the service method that shares its rules with `MintToken` — AC-19a, AC-29
