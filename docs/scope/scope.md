...
### 20. Open internet hardening: login CSRF & control plane policy · done
...
spec [0020](../specs/0020-platform-backup-restore/index.md) · code in `internal/backup`, `internal/store` (`backups.go`, `backupadapter.go`, migration `00004`), `internal/config/backup.go`, `internal/domain/backupreason.go`, `internal/web/backups.go`, `cmd/deployer/restore.go`, `deploy/README.md`
- [x] Design it (spec): `/architect platform backup & restore`
- [x] Build it: `/develop platform backup & restore`
  - [x] The thin thread, end to end first: the `backup_runs` migration with its in flight index, and one run carrying a real snapshot from the live database through age to a real object in the bucket and back to a closed row — AC-1, AC-2, AC-4, AC-4a, AC-4b, AC-5, AC-7, AC-8, AC-11
  - [x] What makes it a backup rather than an upload: the separate closed reason set, the constraint violation told apart from a real write fault, the integrity check on the plaintext, the recipient stanza checked in the age header, and the read back comparing length and checksum — AC-3, AC-4c, AC-6, AC-8a, AC-10
...
### 22. Public edge: tunnel, real certificates & the console hostname · done
...
- [x] Build it: `/develop public edge`
  - [x] The split and the name: the console host setting with its validation and derived URL, the host qualified mux registrations with their catch all, the hidden admin links, and the reserved label set with its reason code — AC-1, AC-2, AC-2a, AC-3, AC-4, AC-5, AC-6, AC-7 · AC-5 was not held when it was driven live on 2026-08-16: the account lockout lived in the JSON handler alone, so the browser, the surface the public edge exposes, applied none of it. Moved into `Service.Login` so both surfaces inherit it, fix `4bd0258`
  ...
  - [x] The tunnel: the namespace peer on the control plane policy, then the tunnel itself with two origins, its own policy, the production certificate, and the health mail on the existing Resend path — AC-8, AC-9, AC-9a, AC-10, AC-11, AC-12, AC-22, AC-22a, AC-23, AC-23a, AC-24, AC-25 · the tunnel runs 2/2 in `deployer-edge` with four ready connectors, and `wildcard-apps` is issued by `letsencrypt-prod`, reports `Ready`, and covers both names, so AC-8 is no longer owed
  - [x] The flip, last and alone: prove everything above from the tailnet with a host header override, then change the two DNS records and retire the `/32` host route — AC-13, AC-14, AC-15a, AC-26 · done 2026-08-16. The flip exposed one defect the earlier host header proof could not reach: the tunnel listed the app wildcard above the console route and a leading `*.` matches the single label `console`, so the console 404d from `ingress-nginx` until the order was swapped
- [x] Verify it: `/check verify public edge` — includes AC-14, the console and an app answered from a machine with no Tailscale · done 2026-08-16 on the deployed digest `sha256:714c06dd`. Everything in `verify.md` that can be observed has been, the reserved name and the tunnel failure mails last. Two criteria stay covered by the suite alone and cannot be driven yet: AC-18 and AC-18a, since the oldest `audit_log` row is four days old and the sweep's window is ninety, and the half of AC-17 about a platform written row, since no path in the platform writes one to `audit_log`. Neither is a failure and neither is fixable by running something today
- [x] Test it: `/test public edge` — the gaps rather than a second suite, since `/develop` already wrote AC tagged tests across 13 files. Added `internal/config/tunneldeployment_test.go` pinning AC-10 (two connectors, the credential sealed with no plain Secret anywhere in `deploy/`, the image pinned by digest) and the AC-23 three file agreement between the container's metrics port, the health Service and the port the tunnel policy permits, plus AC-23a's restart cost and AC-24's independent telling in `internal/tunnelwatch`. Every new assertion was mutation checked: seven deliberate breaks, seven failures. AC-8, AC-9a, AC-13, AC-14, AC-15a, AC-25 and AC-26 stay `/check verify` only, since the fake clientset resolves no names and the test process is behind no proxy
...
spec [0024](../specs/0024-oauth-for-connector-clients/index.md) · code in `internal/identity` (`oauth.go`, `oauthgrant.go`), `internal/store` (migration `00007`, `oauth.go`, `queries/oauth.sql`), `internal/web` (`oauth.go`, `templates/authorize.html`, the fifth `/connect` tab), `internal/httpapi` (`oauth.go`), `internal/mcp` (the `WWW-Authenticate` challenge), `internal/auth` (`connector_grant`) · the live proof against the real Claude desktop app has not run yet and is what `/check verify` is for · originally touched `internal/web`, `internal/httpapi`, `internal/mcp`, `internal/identity`, `internal/auth`, `internal/store` (migration `00007`)
- [x] Design it (spec): `/architect connecting a client that will not hold a token`
- [x] Build it: `/develop connecting a client that will not hold a token`
  - [x] The schema and the grant path: migration `00007` with both tables, the nullable column and the two indexes that carry rules, plus the store transaction and the service method that shares its rules with `MintToken` — AC-19a, AC-29
  - [x] The thin thread end to end: both discovery documents, the `WWW-Authenticate` header, registration, a plain approval page and a token endpoint, proved by adding the connector in the real Claude desktop app and driving one deploy to healthy before anything is thickened — AC-1, AC-2, AC-3, AC-3a, AC-4, AC-9, AC-16, AC-17, AC-19, AC-20, AC-25
  - [x] The refusals that keep it safe: redirect URI validation and the exact comparison rule, the validation order that forbids an open redirect, PKCE, the resource check at both ends, single use codes with the replay revoke, and OAuth error codes throughout — AC-4a, AC-5, AC-10, AC-10a, AC-10b, AC-10c, AC-11, AC-12, AC-16a, AC-18, AC-18a, AC-21, AC-24
  - [x] The person's half: the approval page's content and ordering, the loopback warning, Deny, the CSRF check, the suspension refusal, and the session gate returning to an authorize URL with its query intact — AC-9a, AC-13, AC-13a, AC-14, AC-15
  - [x] The bounds, the trail and the surface: the connector limiter bucket kept off the sign in one, the sweep on the existing daily runner, the `connector_grant` audit row, the concurrency proofs, the fifth `/connect` tab, and the negative registration test in both directions — AC-2a, AC-6, AC-7, AC-8, AC-8a, AC-19b, AC-20a, AC-22, AC-23, AC-25a, AC-25b, AC-26, AC-27, AC-28, AC-30
  - [x] The replay rule PKCE decides: `Grant` revokes on a consumed code only when the replay cannot prove its verifier, so an ordinary retry no longer costs a connector the token it was just issued, spec 0024 AC-16b · three tests in `internal/web/oauth_test.go`, the thief's replay that revokes, the client's own sequential retry that keeps its token, and the raced pair that mints once and leaves that token live
- [x] Verify it: `/check verify connecting a client that will not hold a token` — done 2026-08-16 on branch digest `sha256:77de8773`. The real Claude desktop app added the connector by address alone, registered itself, sent the person to the console to approve, and listed all ten tools; its grant is an ordinary token in `/tokens`. AC-16b's three legs were driven live: the client's own retry and a wrong client id both keep the token, a wrong verifier still revokes, and a raced pair mints once and leaves that token live. The loopback flow was driven against a real listener, so AC-10d's port is observed rather than assumed. Two steps stay undriven and neither leaves a criterion unevidenced: approving as one account while another is signed in (only one account exists), and moving the clock a day forward (a change to the deployment, which this gate does not make). One check was reworded rather than passed: the AC-20a ordinal can never fire by granting one client twice, since AC-19 frees the name in the same transaction, so it was driven with two clients sharing a name
- [x] Test it: `/test connecting a client that will not hold a token` — the gaps rather than a second suite, since `/develop` already wrote AC tagged tests across the OAuth surface and the AC-16b fix shipped with three of its own. Added three to `internal/web/oauth_test.go` covering what that fix left open: a table of four replays that prove the verifier but are wrong in every other way (another client's id, an unregistered one, a foreign redirect URI, somebody else's resource), all refused and all keeping the token, which is what pins the consumed branch ahead of the identity checks so only the verifier ever decides; a replay carrying no `code_verifier` at all, which must revoke rather than read as proof; and a client retrying three times in a row, since the bug this replaced only bit on the second arrival. All five broken variants were mutation checked against the guard removed: five deliberate breaks, five failures
...
- [x] Review it (fresh model): `/check review emailed invites bound to an address` — reviewed on Sonnet 2026-08-16, verdict Approve, no blockers and no majors; the two minors are both missing tests rather than defects, in `docs/reviews/2026-08-16-feat-emailed-invites-bound-to-an-address.md`
- [x] Document it: `/document emailed invites bound to an address` — PR description written from the branch commits and diff, 2026-08-16, covering the binding in the lookup predicate, the taken address guard inside the mint transaction, both surfaces, and the opaque origin fix that rode along
...
- **Client ID Metadata Documents beside dynamic registration**: from spec 0024, which chose registration deliberately and recorded the cost. The MCP draft deprecates registration in favour of a `client_id` that is an HTTPS URL the authorization server fetches, and Anthropic prefers it because registration makes a client register itself on every fresh connection. Adopting it means the control plane pod fetches a URL a stranger chose, so it needs an answer on bounding that fetch before it is worth the table it saves · needs a decision
- **Per tool scopes for a connector**: from spec 0024, which gave a connector token exactly the power of a pasted one. Approving read only access, `get_logs` and `deployment_status` without `deploy_app`, means a scope check in every tool handler and a new member in the closed reason code set, which is its own feature rather than a flag on this one · needs a decision
...
- **A check that the control plane policy's peers match the cluster**: feature 20 shipped without the node address problem this item was written for, because the four `/32` entries were replaced by a Cilium entity rule that needs no editing when a node is added. What is left unenforced is narrower and still real: the in namespace peer selects `app: deployer`, read live off one of only two pod labels in the namespace today, so a third pod that needs the registry fails with the parse test green and the build green. From spec 0019, cheap only if there turns out to be a sensible place to run it, CI having no cluster access and the startup sweep being an odd home for it · needs a decision
