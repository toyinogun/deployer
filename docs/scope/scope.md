...
spec [0024](../specs/0024-oauth-for-connector-clients/index.md) · code will touch `internal/web`, `internal/httpapi`, `internal/mcp`, `internal/identity`, `internal/auth`, `internal/store` (migration `00007`)
...
- [ ] Build it: `/develop connecting a client that will not hold a token`
  ...
  - [ ] The thin thread end to end: both discovery documents, the `WWW-Authenticate` header, registration, a plain approval page and a token endpoint, proved by adding the connector in the real Claude desktop app and driving one deploy to healthy before anything is thickened — AC-1, AC-2, AC-3, AC-3a, AC-4, AC-9, AC-16, AC-17, AC-19, AC-20, AC-25
  - [ ] The refusals that keep it safe: redirect URI validation and the exact comparison rule, the validation order that forbids an open redirect, PKCE, the resource check at both ends, single use codes with the replay revoke, and OAuth error codes throughout — AC-4a, AC-5, AC-10, AC-10a, AC-10b, AC-10c, AC-11, AC-12, AC-16a, AC-18, AC-18a, AC-21, AC-24
  - [ ] The person's half: the approval page's content and ordering, the loopback warning, Deny, the CSRF check, the suspension refusal, and the session gate returning to an authorize URL with its query intact — AC-9a, AC-13, AC-13a, AC-14, AC-15
  - [ ] The bounds, the trail and the surface: the connector limiter bucket kept off the sign in one, the sweep on the existing daily runner, the `connector_grant` audit row, the concurrency proofs, the fifth `/connect` tab, and the negative registration test in both directions — AC-2a, AC-6, AC-7, AC-8, AC-8a, AC-19b, AC-20a, AC-22, AC-23, AC-25a, AC-25b, AC-26, AC-27, AC-28, AC-30
