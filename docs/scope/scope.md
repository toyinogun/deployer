...
spec [0021](../specs/0021-public-edge/index.md) · code in `internal/config`, `internal/web`, `internal/httpapi`, `internal/domain`, `internal/auth`, `internal/store` (migration `00005`), `deploy/`, plus the `ClusterIssuer` in `k3sprox-gitops`
- [x] Design it (spec): `/architect public edge`
- [ ] Build it: `/develop public edge`
  - [ ] The split and the name: the console host setting with its validation and derived URL, the host qualified mux registrations with their catch all, the hidden admin links, and the reserved label set with its reason code — AC-1, AC-2, AC-2a, AC-3, AC-4, AC-5, AC-6, AC-7
  - [ ] Seeing the visitor: the single header derivation shared by both surfaces, migration `00005` with the address on `auth.Audit`, and the daily sweep that nulls it after the retention window — AC-15, AC-15b, AC-16, AC-17, AC-18, AC-18a
  - [ ] The sibling subdomain problem: the `__Host-` session cookie with its plain HTTP fallback, the two accepted origins, and the proof the CSRF pre token stays host scoped — AC-19, AC-20, AC-21, AC-21a
  - [ ] The tunnel: the namespace peer on the control plane policy, then the tunnel itself with two origins, its own policy, the production certificate, and the health mail on the existing Resend path — AC-8, AC-9, AC-9a, AC-10, AC-11, AC-12, AC-22, AC-22a, AC-23, AC-23a, AC-24, AC-25
