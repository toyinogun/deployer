...
Spec 0019 corrects the line above twice. The control plane is not behind ingress-nginx at all, it sits on the `tailscale` ingress class, and the node addresses have to be a peer because containerd pulls images from the in cluster registry as the node rather than as a pod. The peer list is also four, not three: the control plane reads a pushed image back from the registry over the pod network, so `deployer-system` is a peer of itself, and leaving that out breaks every deploy while passing every test.
...
- [ ] Build it: `/develop open internet hardening`
  - [x] The policy, riskiest first: the proxy pod's namespace confirmed live, the ingress only pair with its four peer groups against container ports, and the parse test pinning the shape — AC-11, AC-12, AC-15, AC-16, AC-18
  - [ ] Proved on the real cluster: an app namespace refused on 5000 and 8080, and every caller in AC-14's list still working, ending with a deploy carried to `healthy` rather than a build Job going green — AC-13, AC-14, AC-19
