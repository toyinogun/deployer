...
- [ ] Build it: `/develop open internet hardening`
  - [ ] The policy, riskiest first: the proxy pod's namespace confirmed live, the ingress only v1 pair with its three pod sourced peers against container ports, the Cilium object carrying the node peers by identity, and the parse tests pinning both — AC-11, AC-12, AC-12a, AC-15, AC-18
  - [ ] Proved on the real cluster: an app namespace refused on 5000 and 8080, a pull proved both ways on a node that is not the registry's, and every caller in AC-14's list still working, ending with a deploy carried to `healthy` rather than a build Job going green — AC-13, AC-14, AC-19
