...
- [ ] Build it: `/develop connecting a client that will not hold a token`
  ...
  - [ ] The replay rule PKCE decides: `Grant` revokes on a consumed code only when the replay cannot prove its verifier, so an ordinary retry no longer costs a connector the token it was just issued, spec 0024 AC-16b
