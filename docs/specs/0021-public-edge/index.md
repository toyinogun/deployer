...
- **AC-10**: `cloudflared` runs in its own namespace with two replicas, so a node drain or a rolling update never leaves zero connectors. The tunnel credential exists in the cluster only as a `SealedSecret` and appears in plain text in no repository.
