...
- [ ] everyone who was signed in before this deploy is signed out exactly once → AC-20 · **FAILS, and it takes AC-19's protection with it.** `auth.SessionID` in `internal/auth/session.go` reads **both** cookie names, secure first then plain, so a session minted before the rename still authenticates. Observed: the tailnet host serves `/apps` signed in while the only cookie it holds is a pre rename `deployer_session`, which the current binary cannot have written, since the same binary wrote `__Host-deployer_session` on the console minutes earlier. Nobody was signed out.
  ...
  **Fixed the same day, still owing a live re run.** `SessionID` now takes `secure` and reads the one name `SessionCookieName` selects, exactly as the write does, and both callers pass the value they already held. Pinned by `TestASessionIsReadUnderOneNameOnly` and `TestAParentScopedPlainCookieIsNotASession` in `internal/auth/session_test.go`, and the assertion in `internal/web/cookieorigin_test.go` that used to require both names to resolve is now the opposite assertion. The comment on `SessionID` argued that reading one name "would sign everybody out on every request rather than once", and signing everybody out once is what AC-20 asks for, so that reasoning was the bug. Re run both halves against the deployed digest once CI has published it: a pre rename `deployer_session` must no longer authenticate, and the parent scoped cookie must answer the sign in page rather than `/apps`. Expect the one sign in for everybody that AC-20 always described, this time actually happening
