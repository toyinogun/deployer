...
- **AC-13**: `deploy_app`'s description names the bound in prose: outbound mail on port 25 and the common mining pool ports are closed, and an app that sends mail should use a provider's relay on 587. It names the shape rather than the literal list, so it carries no pool port number and needs no edit when one of them changes. It describes the default configuration and nothing stronger: an operator who narrows `DEPLOYER_APP_EGRESS_BLOCKED_PORTS` makes part of the prose untrue until they widen it again, which is accepted because the list is a bound on abuse rather than a tuning knob.
