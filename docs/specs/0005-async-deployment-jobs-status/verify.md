...
- [x] `reason`/`message`: fail a deploy with a root image → status carries `image_runs_as_root` and its one line, and nothing from the build log. Was deferred to slice 6 (Dockerfile build path), the same blocker spec 0004's AC-10 hit: while `deploy_app` only built through Paketo, which will not produce a root running image, this failure could not be reached through the real path. Closed on 2026-08-13 by spec [0009](../0009-dockerfile-build-path/index.md), steps 9 and 16: a rootful `Dockerfile` deployed against the cluster was refused `image_runs_as_root` before any app object was composed, with no build output in the status response or in the platform log. Pinned afterwards by `TestARootImageFromADockerfileBuildIsRefusedBeforeAnyAppObject` in `internal/reconcile/buildpath_test.go`, the first test that can reach this code through a Dockerfile produced image. The earlier evidence still stands: the failed branch of the status payload test covers the projection, and a real failed deployment on 2026-08-12 reported `reason` `timeout` with its one line message, three timeline entries, no `detail` field, and no release fields
...
- [x] Watchdog budget on resume: restart the control plane mid build → the resumed deployment gets what is left of its budget, not a fresh one → AC-14a. Run 2026-08-12 against the cluster with the budget temporarily shrunk to 300s (a 600s budget outlives every real build, so nothing stays in flight long enough to reach it): the target's build Job was suspended to hold it in `building`, the pod was killed at t+164s, the new pod logged `resuming a deployment left in flight`, and the deployment failed `timeout` at t+300.0078s (`queued` 15:00:14.114280213Z, `failed` 15:05:14.122070403Z), its own `created_at` plus the budget. A fresh budget on resume could not have ended it before t+464s
