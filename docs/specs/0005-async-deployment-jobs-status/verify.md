...
- [ ] Upload `testdata/sample-go`, then call `deploy_app` and time it → returns in well under a second, and the time does not vary with build time → AC-1, AC-19
- [ ] Read the `deploy_app` response → carries `deployment_id`, `name`, `slug`, `url` (`https://<slug>.<DEPLOYER_APP_DOMAIN>`), `state` = `queued`, and no `release_number` or `image_digest` → AC-2
- [ ] Poll `deployment_status` with that `deployment_id` every few seconds → reports `queued`, then a building state, then `healthy` → AC-6, AC-19
