...
- **AC-10d**: The address a match yields is the **requested** one, port and all, and that single value is what the redirect goes to and what is stored on the code for AC-18 to compare against. Yielding the registered form instead is the failure this criterion exists to name, and the build made it: the code went to port 80, where a native client is not listening, and the exchange then refused the client's own address against a stored one carrying no port, so the whole loopback flow was dead while every unit test passed. Nothing is loosened by this, because only the port can differ once AC-10a's equality checks have run. A test asserts the returned value rather than only the boolean, and a second drives approve and exchange through the handlers on an ephemeral port.
...
- **AC-16a**: A code expires 60 seconds after it is written and is single use. A second presentation of a consumed code is always refused with `invalid_grant`, and never mints a second token.
- **AC-16b**: Whether that refusal also revokes the token the first presentation issued is decided by PKCE, not by the fact of a second arrival. The replay revokes when its `code_verifier` does not hash to the stored challenge, and leaves the token alone when it does. What OAuth 2.1's revoke defends against is a code arriving twice because somebody other than the client has it, and on a public client the verifier is precisely the proof of which caller this is: a thief who captured the code from a redirect cannot produce it, and the client retrying its own request always can. The first build revoked on every second arrival, so a lost response or a double submit cost the connector the token it had just been issued, 0.7ms after minting it, and the caller was handed a `200` carrying a credential that was already dead. The verifier alone decides the revoke. A replay whose `client_id`, `redirect_uri` or `resource` does not match the stored code is refused exactly as any other `invalid_grant` under AC-18, and that mismatch does not change whether the first token survives, because a caller proving the verifier is the client whatever else it got wrong.

Three tests, and the ordering of them matters. A replay with a wrong verifier revokes. A **sequential** retry, the first exchange run to completion and then the identical code and verifier presented again, is refused while the first token stays live: that is the case this criterion exists for, and it is the one a build could otherwise fail while passing everything else, since the wrong verifier test was never in question and the race below resolves in the store's conditional update, so whether it reaches this branch at all is left to the scheduler. And the raced pair leaves the minted token live.
...
- **AC-25b**: Every one of those seven registrations names its method. What that produces depends on the pattern, and the criterion says so rather than asking for a refusal this mux cannot give: on the bare pattern the standard mux answers `405` for a wrong method on a path that exists, and on a host scoped pattern the host's own catch all claims the request first and answers `404`, exactly as it already does for `/login`. Both are asserted, because "registered deliberately" is only checkable if the refusals are pinned; the original wording expected `405` on the console host, which was never true of this mux and is not a bound worth loosening a catch all for.
...
- **AC-26**: `/connect` gains a fifth tab for the Claude app and other connectors. It sits after the three named client tabs and **before** the generic MCP JSON one, which stays last: `genericClient` is the final entry in the set, so the last tab is what an unknown or tampered `client` form value resolves to, and that fallback must not be a panel carrying no token and no mint control (AC-17). The ordering is therefore a constraint rather than a preference, and moving this tab means naming the fallback explicitly first. It shows exactly `Config.MCPURL + "/mcp"` and nothing else: no token, no placeholder, and no mint button while it is selected, because this flow issues its own credential. The endpoint follows a configuration swap the way the other four blocks do, asserted by the same style of test.
...
| | `token_id` | TEXT | yes | The token this code issued, so a replay that cannot prove the verifier can revoke it (AC-16b) |
...
- An authorization code is exchanged at most once, enforced by a conditional update rather than by a read (AC-18a). A second attempt is always refused, and costs the token the first one issued only when it cannot prove the PKCE verifier (AC-16a, AC-16b).
- A redirect and the value stored on the code are the address the request asked with, which for a loopback client carries the ephemeral port it is listening on (AC-10d).
- Consuming the code, minting the token and stamping `token_id` on the code are one transaction, so no replay can ever read a row whose `consumed_at` is set while its `token_id` is still null. AC-16b's revoke depends on that: a stamp written outside the transaction reopens a window where a replay is refused because the platform does not know which token to revoke, which looks identical to the verifier having decided it and is not the same thing.
...
- Failure case: an authorization code presented twice refuses the second, and revokes the token the first issued when the replay's verifier is wrong, verifies **AC-16a**, **AC-16b**.
- Failure case: an exchange run to completion, then the identical code and verifier presented again, is refused while the token the first one issued stays live, driven sequentially rather than as a race so it cannot pass by scheduling, verifies **AC-16b**.
- Failure case: two token requests for one code arriving together mint exactly one token and leave it live, driven as a real race, verifies **AC-18a**, **AC-16b**.
- Failure case: a loopback client registered without a port and authorizing with one is redirected to the port it asked with, and exchanges against it, driven through the handlers, verifies **AC-10a**, **AC-10d**.
...
6. [x] The surface and the proof nothing else moved: the fifth `/connect` tab with its `<noscript>` fallback, the negative registration test in both directions with the method refusals distinguished, and the confirmation that the limiter's `429` and the suspended tool result are unchanged, satisfies **AC-2a**, **AC-25a**, **AC-25b**, **AC-26**, **AC-27**, **AC-30**.
7. [x] What driving the built thing found, since the Tracer Bullet's own proof step was the last thing to run rather than the first: the loopback match yields the requested address, satisfies **AC-10d**.
8. [ ] The replay rule PKCE decides, which is a behaviour change rather than a correction: `Grant` checks the verifier before revoking on a consumed code, the sequential retry test that is the only deterministic proof of it goes in first, and the race test asserts the surviving token is live rather than only that one came back, satisfies **AC-16b**.
...
**Negative and tradeoffs**, added on the 2026-08-16 amendment:
- The replay revoke now depends on PKCE rather than on arrival alone (AC-16b), which is a deliberate step back from the strictest reading of OAuth 2.1. A thief holding both the code and the verifier keeps the token alive where the old rule would have killed it. That case was already lost, because whoever holds both can simply win the race, and the rule it replaces was costing real clients their credential on an ordinary retry.
