...
- **CSRF**: a per session synchroniser token on every authenticated POST, plus an `Origin` and `Sec-Fetch-Site` check on every POST. Accepted knowingly: the pre authentication posts carry no synchroniser token, because there is no session to bind one to and adding a pre session cookie to defend against login CSRF is more moving parts than the risk earns on a tailnet only internal tool. They keep the header check, the shared rate limit bucket, and `SameSite=Lax`. **Reversed by [spec 0019](../0019-open-internet-hardening/index.md)**, which built the pre session cookie once the wildcard stopped being tailnet only. What changed was the premise, not the arithmetic: apps deployed by strangers hold sibling hostnames under the same wildcard, which is a threat this spec did not have.
