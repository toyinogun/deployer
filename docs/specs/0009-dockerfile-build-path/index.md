...
7. [x] Add `testdata/sample-dockerfile` and prove the thin thread on the real cluster: it deploys through BuildKit, `testdata/sample-go` still deploys through Buildpacks, satisfies **AC-17**. Proved live on 2026-08-13, see step 16.
8. [x] Project `build_path` onto the `deployment_status` payload, satisfies **AC-5**.
9. [x] Close the refusals: `build_failed`'s message no longer names one engine and `deploy_app`'s description carries the rule. The live half is proved too: a rootful `Dockerfile` fails as `image_runs_as_root` and a failing `RUN` as `build_failed`, with no build output in either response or in the platform log, satisfies **AC-12**, **AC-13**, **AC-14**.
...
12a. Permit the new namespace in the `deployer` AppProject, which lives in the separate `k3sprox-gitops` repository (`apps/deployer.yaml`) and is only mirrored here as `deploy/gitops/argocd-application.yaml`. Its `destinations` list names every namespace this project may write to, and the new namespace ships a RoleBinding and a NetworkPolicy pair, so without an entry ArgoCD refuses them. This is not a partial failure: validation covers the whole operation, so an unlisted destination stops every object in `deploy/` from applying, including the namespace itself and the `deployer-builds` level change. This step was missing from the plan when step 12 landed, and the sync failed exactly this way on 2026-08-13. It is a separate repository, so it merges on its own before the cluster can hold anything else here. The fence is unchanged in kind: one more platform namespace this repo owns, no wildcard, and an `app-<slug>` namespace made at runtime is still out of scope by construction, so spec 0003's AC-15 still holds.
...
16. [x] Re run the live proof that step 7 could not complete. Done on 2026-08-13 against the cluster: `sample-dockerfile` built through BuildKit in 15s and answered 200 on its hostname, `sample-go` built through Buildpacks in 34s and answered 200, a rootful `Dockerfile` was refused `image_runs_as_root` before any app object, a failing `RUN` ended `build_failed`, and all four rows reported their `build_path`. The Paketo Job landed in `deployer-builds` and the three BuildKit Jobs in `deployer-builds-dockerfile`. Satisfies **AC-1**, **AC-7**, **AC-7b**, **AC-17**, **AC-12**, **AC-13**, **AC-14**.
