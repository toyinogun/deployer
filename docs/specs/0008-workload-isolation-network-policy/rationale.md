...
The engineer chose not to police `deployer-system` in this slice. That leaves any workload on the cluster able to reach the platform API, which is guarded by tokens rather than by network, so it is a defence in depth gap rather than an open door. It is recorded as the first Follow-up. That follow up was taken up by [spec 0019](../0019-open-internet-hardening/index.md) once the wildcard was about to face the open internet, which is the point at which "guarded by tokens" stopped being the whole answer. Worth reading beside the paragraph above about the ingress controller's host networking: the same class of assumption, a namespace selector matching where traffic really comes from, is exactly what went wrong twice in 0019, and both times every test still passed.
