...
- [ ] `go test -race ./...` → every package passes → AC-1, AC-2, AC-8, AC-9, AC-10, AC-17
- [ ] `go test -race ./internal/source/` → every hostile archive (absolute path, `..`, symlink, hardlink, device, fifo, too many entries, gzip bomb, truncated) is rejected and nothing lands outside the extraction root → AC-8
- [ ] `go test -race ./internal/build/` → the composed Job is one attempt, non root, all capabilities dropped, `RuntimeDefault` seccomp, no service account token, and pushes to the platform's chosen tag → AC-7, AC-9
- [ ] `go test -race ./internal/store/ -run Bootstrap` → seeding three times leaves one account and one live token, and rotating the token revokes the old one → AC-1
...
- [ ] Bootstrap seed, secrecy: `kubectl -n deployer-system logs deploy/deployer | grep "$DEPLOYER_BOOTSTRAP_TOKEN"` → no match, at any log level → AC-1
...
- [ ] Upload `expires_at`: post an upload and expect `expires_at` to be exactly one hour after `created_at`, not a fixed clock time → AC-2
...
- [ ] Build namespace (AC-7): `kubectl get ns deployer-builds -o jsonpath='{.metadata.labels}'` shows `pod-security.kubernetes.io/enforce: restricted`
