...
- **AC-12**: The allow policy permits ingress from exactly three kinds of source, and nothing else, each with its own port list rather than a bare peer, because a peer with no `ports` clause means every port in the namespace:
...
- **AC-14**: With the policy applied, the console still answers over the tailnet, a build still completes through both build namespaces, and an app still pulls its image on a node.
- **AC-15**: A Go test in `internal/config` parses both policy files and pins their whole shape, the way `builds-networkpolicy.yaml` is already pinned: ingress only, the exact peer list, the exact ports, and no egress rule. Widening any of it fails the suite.
...
- Live proof nothing broke: console, both build paths, and an image pull. Verifies **AC-14**.
...
2. The policy pair as static YAML in `deploy/deployer-system-networkpolicy.yaml`, ingress only, with the three peer groups each carrying its own port list against container ports, the node comment, and its kustomization entry. Satisfies **AC-11**, **AC-12**, **AC-16**.
3. The parse test in `internal/config` pinning the whole shape beside the existing build policy tests: ingress only, the exact peers, the exact ports, no egress rule. Satisfies **AC-15**.
4. Live on the cluster: the probe app refused on 5000 and 8080, then the console, both build paths, and an image pull all confirmed still working. If the node ipBlock turns out not to match what Cilium sees, stop and route back through `/architect` rather than reaching for a CiliumNetworkPolicy here. Satisfies **AC-13**, **AC-14**, **AC-19**.
...
- The policy is one more thing that can take the platform down through a mistake, and unlike the app policies it has no run time composer to test.
