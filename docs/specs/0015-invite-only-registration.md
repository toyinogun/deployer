...
- **AC-14**: A raw invite code exists in exactly two places the platform controls, the admin response that minted it and the bootstrap log line. It is never in the database, never in another platform log line, never in an audit row, and never in a mailed message. The register page sends `Referrer-Policy: no-referrer`, so the code in the query string does not travel onward from the browser.
