...
The attack that ends a platform is not the one that reads the backups, it is the one that deletes them, so the design wants a control plane that cannot destroy its own history. The first draft of this spec put that in the credential, scoped to read and write but not delete. Checking it against the provider while writing the setup steps showed R2 cannot express it: its API token presets are Admin Read and Write, Admin Read only, Object Read and Write, and Object Read only, with no per verb editor, and the Object Read and Write preset carries `DeleteObject`.

The protection moved to the bucket instead, as a 7 day object retention rule. This is the better answer rather than a consolation. A narrowly scoped credential only holds while nobody issues a wider one, and issuing a wider one is a dashboard click that leaves no trace in this repository. Retention refuses the delete regardless of who is asking with what. Seven days rather than thirty because the retention and the 30 day expiry have to coexist, and locking for as long as the expiry leaves two rules contending at the boundary; a week is far longer than it takes to notice something is wrong.

Cleanup at 30 days stays a bucket lifecycle rule, so no code in the cluster ever issues a delete at all.
