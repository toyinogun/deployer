...
- [x] `kubectl -n deployer-system exec deploy/deployer -- ls -la /data/backup-tmp/` mid run, immediately after pressing the button → two files, both owned by the pod's uid: the snapshot at mode 600, and SQLite's own rollback journal beside it, which SQLite creates and which is gone before the snapshot call returns → AC-2, AC-4b
- [x] Delete the pod mid run (`kubectl -n deployer-system delete pod <pod>`), let it come back, then list `/data/backup-tmp/` → empty, and the run that was going is `failed: stranded` on the page → AC-2a, AC-9
...
- [x] `ps aux | grep restore` while a restore runs → no age identity appears in the process listing → AC-23
...
- [x] Visit `/admin/backups` signed out, and signed in as a non admin → exactly what the existing admin gate gives on `/admin/accounts`, and no hint the page exists → AC-17
- [x] With backups unconfigured, visit `/admin/backups` → a stated "not configured" panel, not an empty table, and no run button → AC-18
...
- [x] Read the whole `/admin/backups` page source as a non admin and as a signed out visitor → no object key, bucket name, endpoint, or credential appears anywhere → AC-14
