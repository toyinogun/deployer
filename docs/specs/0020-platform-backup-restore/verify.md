...
- [ ] `kubectl -n deployer-system logs deploy/deployer | grep backup` right after a rollout → the schedule started, naming the interval; no line names a bucket, a key, or a credential → AC-12, AC-14
- [ ] `kubectl -n deployer-system exec deploy/deployer -- ls -la /data/backup-tmp/` while no run is going → the directory exists and is empty → AC-2, AC-2a
...
- [ ] `rclone ls`/`aws s3 ls` the bucket under `db/` → the object is there, its name is `db/<YYYYMMDDTHHMMSSZ>-<run id>.age`, and its size matches the page → AC-5, AC-6
- [ ] `head -c 30 <the downloaded object>` → it begins `age-encryption.org/v1`, and nothing in it is readable SQLite → AC-4
- [ ] `go run ./cmd/deployer restore -key <key> -identity <file> -out ./restored.db` on your own machine → it writes the file and reports success → AC-23
- [ ] Run that same command again with the same `-out` → refused because the destination exists, and the existing file is untouched → AC-24
- [ ] Run it with `-identity` pointing at a file that is not an age identity → refused, and the error echoes no part of the file → AC-23
- [ ] `sqlite3 ./restored.db "SELECT COUNT(*) FROM accounts"` → matches what the platform held when the backup was taken → AC-3, AC-23
- [ ] `ps aux | grep restore` while a restore runs → no age identity appears in the process listing → AC-23
- [ ] Set only five of the six `DEPLOYER_BACKUP_*` values on a scratch deploy → the pod refuses to start, naming the missing one → AC-16
- [ ] Remove all six on a scratch deploy → the pod starts, logs one warning naming all six, and every other page works → AC-15
...
- [ ] Press **Back up now** → the run appears as `succeeded`, `by hand`, with a size and an object key → AC-19, AC-21
- [ ] Press **Back up now** twice quickly → the second is refused, the page says a backup is already running, and the bucket gains one object rather than two → AC-8, AC-20
- [ ] `SELECT * FROM audit_log WHERE target_type='backup'` after both presses → two rows, the allowed one and the refused one → AC-22
- [ ] `SELECT trigger, triggered_by FROM backup_runs` → the pressed run is `manual` with your account id, the scheduled one is `schedule` with null → AC-21
- [ ] Break the credential (a wrong secret key on a scratch deploy), press the button → the run ends `failed: upload_failed`, one mail arrives naming only that code, and the mail carries no bucket name, object key, or configuration value → AC-13, AC-14
- [ ] Fix the credential, press again → one recovery mail arrives. Press a third time → no mail at all → AC-13
...
- [ ] While there, confirm `/data/uploads` did not come back and nothing depends on it having → AC-29
